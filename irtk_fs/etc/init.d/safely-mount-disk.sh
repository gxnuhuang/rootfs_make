...
if [ $2 -eq 1 ]; then
	...
	sleep 1
	mkfs.vfat -n update /dev/mmcblk"$1"p1
	mkfs.vfat -n static /dev/mmcblk"$1"p2
fi
...
/bin/mount -t auto -o rw /dev/mmcblk"$1"p1 /mnt/firmware
if [ $? -ne 0 ]; then	
...
/bin/mount -t auto -o rw /dev/mmcblk"$1"p2 /mnt/storage
if [ $? -ne 0 ]; then	
...
if [ $? -ne 0 ]; then	
	...
	/bin/mount -t auto -o rw /dev/mmcblk"$1"p3 /mnt/platform
	touch /mnt/platform/etc/fixpartition
fi
