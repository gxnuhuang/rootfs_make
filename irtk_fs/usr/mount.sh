...
automount() {	
	...
	then
		...
		if ! touch /media/$name/rwtest
		then
			echo "mount sd card read only" > /tmp/mount.log
			$UMOUNT /media/$name
			$MOUNT -t auto $DEVNAME "/media/$name"
			
		else
			echo "mount sd card read write enabled" > /tmp/mount.log
			rm /media/$name/rwtest
		fi
